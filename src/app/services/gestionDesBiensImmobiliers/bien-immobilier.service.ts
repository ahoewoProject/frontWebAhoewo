...
export class BienImmobilierService {
  ...
  activerBienImmobilier(id: number) {
    return this.httpClient.get<BienImmobilier>(this.url + 'activer/bien-immobilier/' + id);
  }

  // url: http://localhost:4040/api/desactiver/bien-immobilier/{id}
  desactiverBienImmobilier(id: number) {
    return this.httpClient.get<BienImmobilier>(this.url + 'desactiver/bien-immobilier/' + id);
  }

  // Affichage du nombre d'occurrences d'agence immobiliere.
  // url: http://localhost:4040/api/count/biens-immobiliers/
